 ...
 const navigate = useNavigate()
  ...
  return (
    <div className='min-h-screen w-full px-4 py-9 bg-gray-50 '>
      ...
      {
        userData?.enrolledCourses?.length === 0 ? (<p className='text-gray-500 text-center w-full'>You have not Enrolled in any course yet</p>) 
            ...
            <div key={index} className='bg-white rounded-2xl shadow-md overflow-hidden border'>
              <img src={course?.thumbnail} alt="" className='w-full h-40 object-cover'/>
               <div className='p-4'>
                 ...
                 <h1 className='px-[10px] border-2 text-center py-[10px] bg-black text-white rounded-[10px] text-[15px] font-light  cursor-pointer mt-[10px] hover:bg-gray-600' onClick={() => navigate(`/viewlecture/${course._id}`)}>Watch Now</h1>
               </div>
            </div>
          ))}
        </div>)
