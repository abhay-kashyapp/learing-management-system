      ...
      <div className="w-[100%] min-h-screen p-4 sm:p-6   bg-gray-100">
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3 ">
         <div className='flex items-center justify-center gap-3'><FaArrowLeftLong className=' w-[22px] h-[22px] cursor-pointer'       onClick={() => navigate("/dashboard")} />
            <h1 className="text-xl font-semibold">All Created Courses</h1>
          </div>
    ...
    <tbody>
     {creatorCourseData?.map((course,index) =>(
      ...
      <tr key={index} className="border-b hover:bg-gray-50 transition duration-200">
        <td className="py-3 px-4 flex items-center gap-4"> 
          {course?.thumbnail ?<img src={course?.thumbnail} className='w-25 h-14 object-cover rounded' alt="" /> : <img src={img} className='w-25 h-14 object-cover rounded-md ' alt=''></img>}
     ...
     <div className="md:hidden space-y-4">
       ...
       <div key={index} className='bg-white rounded-lg shadow p-4 flex flex-col gap-3'>
        ...
        { course?.thumbnail? <img src={course?.thumbnail} alt="" className="w-14 h-14 object-cover rounded-md object-fit"/> : <img src={img} alt="" className="w-14 h-14 object-cover rounded-md object-fit"/>}
