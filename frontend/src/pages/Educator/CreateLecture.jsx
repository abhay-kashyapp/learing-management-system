...
//      toast.warn("Lecture title cannot be empty.");
//      return;
//    }
//    setLoading(true);
//    try {
...
//  }
  ...
  setLoading(true);
  try {
    console.log("Sending lectureTitle:", lectureTitle, "courseId:", courseId);
      ...
      <div className="bg-white shadow-xl rounded-xl w-full max-w-2xl p-6"  >
        ...
        { lectureData?.map((lecture,index)=>(
          <div key={index} className=' bg-gray-100 rounded-md flex justify-between items-center p-3 text-sm font-medium text-gray-700'>
            ...
            <FaEdit className='text-gray-500 cursor-pointer hover:text-gray-700' onClick={()=>navigate(`/editlecture/${courseId}/${lecture._id}`)}/>
          </div>
         ))}
        </div>
